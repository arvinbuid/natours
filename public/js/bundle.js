(() => {
  ...
  var displayMap = (locations) => {
    ...
    locations.forEach((loc) => {
      ...
      L.marker(reversedArr, { icon: myIcon }).addTo(map).bindPopup(`Day ${loc.day}: ${loc.description}`, {
        maxWidth: 200,
  ...
  var showAlert = (type, msg) => {
    ...
    window.setTimeout(() => hideAlert(), 2e3);
  };
  ...
  var updateSettings = async (data, type) => {
    try {
      const url = type === "password" ? "http://localhost:3000/api/v1/users/updateMyPassword" : "http://localhost:3000/api/v1/users/me";
      const res = await axios_default({
        method: "PATCH",
        url,
        data
