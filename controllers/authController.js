...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    expires: new Date(
      Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000 // H * M * S * MS
    ),
    httpOnly: true
  };
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); // +password is used to display a !select field
...
exports.logout = (req, res) => {
  // Set similar cookie but without token
...
exports.isLoggedIn = async (req, res, next) => {
  try {
    if (req.cookies.jwt) {
      ...
      res.locals.user = currentUser; // add currentUser into res.locals
      return next();
    }
  } catch (err) {
    return next();
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('The user with this email address does not exists.', 404)
    );
  }
