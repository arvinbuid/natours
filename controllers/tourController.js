...
exports.aliasTopTours = (req, res, next) => {
  ...
  req.query.fields = 'name,difficulty,duration,price,summary,ratingsAverage';
  next();
};
...
// Upload multi image multer upload middleware
...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  await Promise.all(
    req.files.images.map(async (curFile, i) => {
      const filename = `tour-${req.params.id}-${Date.now()}-${i + 1}.jpeg`;

      await sharp(curFile.buffer)
        .resize(2000, 1333) // 2000 x 1333
        .toFormat('jpeg')
        .jpeg({ quality: 90 })
...
exports.getMonthlyPlan = catchAsync(async (req, res, next) => {
  ...
  const plan = await Tour.aggregate([
    ...
    {
      $project: { _id: 0 } // exclude id field
    },
    {
...
// /tours-within/123/center/34.05986842414883, -118.24700605840204/unit/mi
